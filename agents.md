...
- Mocha defaults live in the repo-root `.mocharc.cjs` (currently `timeout: 10000`). Every Mocha workspace `cd ../..` first, so they all pick it up — don't re-declare shared options per package. Mocha's own 2s default timeout is too tight here: hooks that build databases or synced peer pairs get starved past it under concurrent load, surfacing as a phantom `Timeout of 2000ms exceeded`. Override per run with `--timeout <ms>`

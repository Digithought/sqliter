...
5.  **Benchmark Suite (`bench/`)**
	*   Standalone benchmark harness run via `yarn bench`. Measures parser, planner, execution, and mutation throughput across 27 benchmarks, including text-comparison-sensitive `order by` / `distinct` / text-primary-key cases (the `BINARY` collation comparator is the engine's hottest).

...
5.  **Benchmark Suite (`bench/`)**
	...
	*   `yarn bench --baseline <file>` compares against a previous result, color-codes regressions (>20% red) and improvements (>10% green), and exits non-zero on regressions. `yarn bench --filter <substring>` runs only the benchmarks whose `suite/name` matches.

...
5.  **Benchmark Suite (`bench/`)**
	*   Standalone benchmark harness run via `yarn bench`. Measures parser, planner, execution, and mutation throughput across 46 benchmarks, including text-comparison-sensitive `order by` / `distinct` / text-primary-key cases (the `BINARY` collation comparator is the engine's hottest).
	*   **Storage backends are a dimension, not separate suites.** The 19 `execution` and `mutation` *workloads* are defined once, in `bench/workloads/`, and `expandBackends` (`bench/lib/backends.mjs`) measures each against every backend in one run — hence 46 benchmarks from 27 workloads. The default backend (the in-memory vtab module) publishes the bare name; every other appends `@<id>`, so `full-scan-10k` and `full-scan-10k@store-mem` are two readings of one workload and every pre-existing name still means what it did. `store-mem` is `StoreModule` isolation-wrapped over an in-memory key-value provider — the `yarn test:store` path's performance coverage. It reaches `@quereus/store` through one lazy import (`bench/lib/store-counters.mjs`) so an unbuilt store package makes those rows *skip with a reason* instead of killing suite enumeration for the parser and planner suites too.

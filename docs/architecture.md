...
5.  **Benchmark Suite (`bench/`)**
	...
	*   Records results to timestamped JSON files in `bench/results/` (gitignored). Failed benchmarks are recorded in a separate `failures` array, never as zero-valued entries, so a failed run cannot be mistaken for a fast one.

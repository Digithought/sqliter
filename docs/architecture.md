...
5.  **Benchmark Suite (`bench/`)**
	*   Standalone benchmark harness run via `yarn bench`. Measures parser, planner, execution, mutation and store-layer throughput across 57 benchmarks, including text-comparison-sensitive `order by` / `distinct` / text-primary-key cases (the `BINARY` collation comparator is the engine's hottest).
	*   **Storage backends are a dimension, not separate suites.** The 19 `execution` and `mutation` *workloads* are defined once, in `bench/workloads/`, and `expandBackends` (`bench/lib/backends.mjs`) measures each against every backend in one run — hence 57 benchmarks from 38 entries. The `store` suite is deliberately **outside** that dimension — it calls `@quereus/store` key functions directly, with no query and so no storage engine to swap, so its names carry no `@` suffix. The default backend (the in-memory vtab module) publishes the bare name; every other appends `@<id>`, so `full-scan-10k` and `full-scan-10k@store-mem` are two readings of one workload and every pre-existing name still means what it did. `store-mem` is `StoreModule` isolation-wrapped over an in-memory key-value provider — the `yarn test:store` path's performance coverage. It reaches `@quereus/store` through one lazy import (`bench/lib/store-counters.mjs`) so an unbuilt store package makes those rows *skip with a reason* instead of killing suite enumeration for the parser and planner suites too.
	*   **One process per benchmark.** `bench/run.mjs` is a parent orchestrator that forks `bench/child.mjs` once per benchmark, strictly sequentially; the parent imports suite modules for their metadata but never runs a benchmark itself. This exists because the instruction interpreter shares call sites across query shapes, so in a shared process a benchmark's number depends on which benchmarks ran before it — measured at 0.37x-1.66x for the same benchmark purely by position. Children report raw samples over IPC; the parent derives every statistic (`bench/lib/stats.mjs`) and enumerates suites (`bench/lib/discover.mjs`); the timing policy the worker follows is `bench/lib/calibrate.mjs`, kept out of the worker so it is reachable from a test (`test/bench-calibration.spec.ts`). A benchmark that throws, exits, or hangs fails only itself: the rest still report, its row is marked `FAILED`, and the run exits non-zero.
	*   **Each benchmark calibrates its own sample count.** No benchmark definition carries `iterations` or `warmup`; the worker warms up by elapsed duration (~250 ms), measures the warmed `fn` to pick an inner batch size that puts one sample above clock resolution, then buys as many samples as a ~1 s time target affords — so a 4 µs parser call (batch ~250, 500 samples) and a 110 ms bulk insert (batch 1, ~9 samples) both get comparable statistical weight. Every knob lives in one commented `CALIBRATION` object in `bench/lib/calibrate.mjs`. Setting `iterations` or `warmup` still works and pins that benchmark to a fixed count, reported as `pinned`; it is an escape hatch for a benchmark whose per-call cost changes as it runs, and a pinned benchmark forfeits a trustworthy spread figure. Batching means a batched benchmark's `fn` must be repeatable back-to-back without its `setup`, and its reported min/max are batch means rather than per-call extremes.
	*   **Every row reports its own dispersion.** The table's `Spread` column is relative interquartile range — `(p75 - p25) / median` — chosen over a coefficient of variation because these distributions carry GC and tier-up outliers that inflate a standard deviation without describing the typical iteration. It replaced a `P95` column that, at nearest-rank on these sample counts, printed the same number as `Max`. A row above 20% is marked `unstable` and carries `stable: false` in the JSON: its own samples disagree by more than any change it could be used to gate. **This measures within-run noise only** — a whole run displaced by background load still reads as tight, which is why a low spread is not evidence that two runs are comparable.
	*   Records results to timestamped JSON files in `bench/results/` (gitignored), including per-benchmark `samples`, `batch`, `warmup`, `pinned`, `spread_pct` and `stable` alongside the timings. Failed benchmarks are recorded in a separate `failures` array, never as zero-valued entries, so a failed run cannot be mistaken for a fast one.
	*   **Comparison is noise-aware, and refuses to be silent about what it could not compare.** `yarn bench --baseline <file>` (or `--baseline latest`) judges each delta against a floor built from *both* runs' spreads — `max(5%, sqrt(current² + baseline²))` — and prints that floor beside the delta; a regression must clear both the floor and a 10% minimum before it turns red and fails the run. A benchmark unstable in either run is printed and explicitly *not* gated. **The floor sees within-run spread only**, so it does not catch a whole run displaced by background load; on a loaded machine that displacement can still gate. See *Noise floor* in the benchmarking doc before treating the exit code as a build gate. Every run also records the machine that produced it (CPU, cores, memory, platform, OS release, architecture, Node and V8, commit, dirty tree), and a mismatch on any material axis prints a banner above the table rather than letting two laptops' numbers compare quietly. New, missing, filtered-out and failed benchmarks are four distinct reported outcomes plus a counted summary. `--json` puts the result object on stdout and everything else on stderr. **Full detail, including the exit-code contract and how to add a benchmark, is in [Benchmarking](benchmarking.md)** — do not duplicate it here.
	*   **Counters are compared exactly, where timings are compared against noise.** A benchmark that runs a statement also declares an untimed `counters()` pass, returning the `WorkCounterSnapshot` described in [runtime-work-counters.md](runtime-work-counters.md). Those are machine-independent integers, so the comparison diffs them field by field with no noise floor and no minimum delta, reports them in their own column, and never gates on them — deliberately, so a plan change stays visible on a loaded machine where wall-clock says nothing. `--no-counters` skips the pass. A backend that can instrument its storage layer reports a nested `{ engine, store }` block instead, where `store` counts round trips per physical store — the number a change to batched row resolution would move while leaving every engine counter untouched. Reads only, and only as facts about the run that produced them; see [Benchmarking](benchmarking.md).
	*   Suites may declare `ratioGuards` — a within-run bound on one benchmark's median against another's, which catches a plan-shape regression from a single run with no baseline file at all.

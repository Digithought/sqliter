...
| `expectedLatencyMs` | static field | engine-side fallback (`0`) | 0 | provider-declared (`KVStoreProvider.expectedLatencyMs`, resolved once at construction; `0` when the provider declares nothing, which every in-tree provider does) | forwards underlying | neither declares one — see the `NOTE:` at each plugin's `costProfile` site for why |

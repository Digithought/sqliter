...
| `readCommittedSnapshot` | static field | engine-side fallback (`false` — the read keeps the serialized path) | `true` (immutable layers; single-pointer commit publish; pinned unregistered read connection) | `false` (shared cached table per key drops the option; `query` merges pending ops over the committed store) | `false` **regardless of underlying** — one memoized underlying handle per table is re-served to committed reads, and the overlay flush applies through it incrementally | via store (`false`) |

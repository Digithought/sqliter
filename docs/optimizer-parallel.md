...
## Batched index-nested-loop (one-branch batched fan-out)

The fan-out lookup join's `batched` outer mode (`runFanOutLookupJoinBatched` — many lookups in flight under one global budget, rows re-ordered back into outer order) is also how the engine overlaps the per-outer-row seeks of an **index-nested-loop join** against a high-latency module. `rule-join-physical-selection` offers each index-nested-loop orientation whose seek provably returns at most one row per outer row as a **one-branch `FanOutLookupJoinNode`** — the rebuilt seek under a `Filter` carrying the ON condition, in `atMostOne-inner` / `atMostOne-left` mode — and prices it with `batchedIndexNestedLoopJoinCost` alongside nested-loop, hash, merge and the serial index-nested-loops. The full design (the at-most-one proof, construction, tie order, what is excluded) lives in [Optimizer Joins § When it becomes a fan-out](optimizer-joins.md#when-it-becomes-a-fan-out-batched-seeks).

What this track contributes is the **shared gate**: the selection rule builds the serial one-branch node and hands it to `toBatchedOuter` (exported by `rule-fanout-batched-outer.ts`), the one function that owns every batched gate — `branchCount < outerBatchConcurrency`, slowest-branch latency ≥ `batchedOuterThresholdMs`, outer estimate ≥ `batchedOuterMinRows`, outer `concurrencySafe === true`, no side effects on the outer or any branch — and the `EagerPrefetchNode` wrap that isolates the outer pump from the shared `RuntimeContext`. A decline means no batched candidate; a serial one-branch fan-out is never planned. `ruleFanOutBatchedOuter` itself is now a thin wrapper over the same function, so the two recognition paths cannot drift, and its idempotence gate (`outerMode === 'batched'`) is what lets it re-visit a node the selection rule already flipped. Inert on every in-tree module (`expectedLatencyMs = 0`) — the selection rule does not even construct the node at zero latency — so the golden-plan sweep is unchanged.

**Tuning knobs** (`OptimizerTuning.parallel`): the same as *Fan-out batched outer* — `outerBatchConcurrency` and `maxOuterReadAhead` additionally fix the price's in-flight count, `min(outerBatchConcurrency, maxOuterReadAhead)` (the driver's `deriveReadAhead` at one branch; the coupling is stated at both ends).

...
**Base-table row estimates.** `TableReferenceNode.estimatedRows` — the number every full-relation access node (`SeqScanNode`, `IndexScanNode`, `RetrieveNode`) inherits as its scan cardinality, and the number `rule-select-access-path` passes down as `BestAccessPlanRequest.estimatedRows` for the module to price against — is `table.statistics?.rowCount`: the count `ANALYZE` last collected, and `undefined` when the table has never been analyzed. There is no third spelling — a never-analyzed table is *unknown* (`undefined`), an analyzed empty one is a measured `0`, and consumers apply their own default only on `undefined` (`CatalogStatsProvider.tableRows` hands the unknown case to `NaiveStatsProvider`, default 1000). The logic lives in one place, `catalogRowCount` (`planner/stats/table-cardinality.ts`), which both the node getter and `CatalogStatsProvider.tableRows` call, so the base-cardinality number used for scan costing and the number used for selectivity's denominator cannot disagree.
...
**Where a module's own size fits.** `catalogRowCount` reads the catalog only; nothing calls `VirtualTable.getStatistics()` during planning, so a module's live size never reaches the engine-side cost model between `ANALYZE`s. What a module *can* do is answer for its own access path: `rule-select-access-path` passes the catalog number down as `BestAccessPlanRequest.estimatedRows` (`undefined` for a never-analyzed table — "unknown"), and a module that knows its size may substitute it there — `StoreModule` does, from the row count its write paths maintain. That closes the access-path half only; join ordering, cache thresholds and sort costs still read the catalog and still need an `ANALYZE`. Note what a module must NOT read into `rows` in the other direction: it is an estimate the engine costs with, and `rows: 0` says nothing beyond "very few rows" — the access is still read and its filters still enforced. A module that has *proven* its claimed predicate unsatisfiable says so with the separate `BestAccessPlanResult.provablyEmpty` flag, on which `selectPhysicalNode` replaces the access with an empty relation. See [Module authoring § Row counts](module-authoring.md).
...
Two things a *consumer* of these numbers has to know. **Not every node relays.** The single-source operators and the join family do; a set operation (`union`/`union all`/…), an `AsyncGatherNode`, a CTE reference and the data-modifying nodes stamp nothing, so the count still dies above them (backlog `debt-row-estimates-die-at-set-operations`). Read a missing estimate as *unknown* and fall back, rather than treating the subtree as small. **`undefined` is the only spelling of unknown.** A never-`ANALYZE`d table's scan reports `undefined`, and that `undefined` travels the length of the plan; a numeric estimate — including a real 0, from an analyzed empty table — is a measurement (or a formula over one) and may be read as a magnitude. The corollary binds *relaying* nodes: a formula must not floor a genuine 0 up to 1 (`aggregateRowsFrom` returns 0 groups for 0 source rows — an empty table cannot produce a row), and must pass `undefined` through as `undefined` rather than substituting a confident-looking number of its own.

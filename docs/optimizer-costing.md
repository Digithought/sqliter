...
Today the manager only records the crossing (`isStale`) and logs it once; acting on it is the scheduled-refresh layer's job. `auto_analyze` gates the whole mechanism and defaults on.

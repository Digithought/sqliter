...
- *Group-key pinned* — when a query `where` constant-pins (`g = 1`, `g is null`) or equates (`g₁ = g₂`) a group key **and** there are ≥2 group keys, the matcher forgoes (`group-key-pinned`). This guard was written for a base-path divergence that no longer exists: `rule-groupby-fd-simplification` used to drop the functionally-determined group column and re-emit it as a picker `min` at a *shifted* output position, so the base's column order differed from the rewrite's. The rule now caps a permuting rewrite with an order-restoring `Project` (see its header comment), so both sides deliver select-list order and the guard only costs coverage. Retiring it — with the base-vs-view positional agreement evidence that demands — is `mv-group-key-pinned-guard-obsolete`. (Range / `in` residuals create no determining FD and were always eligible.)

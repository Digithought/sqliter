...
- A query with aggregates **or a `having`**, and **no** `group by`, has one implicit group, and the same restriction applies to it with the empty set of grouping keys: every clause above the aggregation may name aggregates only. `select count(*) from t limit b` and `select count(*) as c from t having (select max(u.a) from u where u.b = t.b) = 'x'` are both rejected with the same message, which names the `group by` clause even though the query has none. See [§3.5](#35-order-by-clause) for which `order by` shapes sit above the aggregation and which sort the input rows below it.

...
**A `having` makes the query an aggregate query on its own.** A `having` with neither aggregates nor a `group by` is not a second `where`: as in SQLite and PostgreSQL, the query has one implicit group over all input rows and `having` filters that single group, so it returns at most one row. Everything above the aggregation is then subject to the restriction above with an empty set of grouping keys — the implicit group carries no base-table columns at all, so `select a from t having a = 'x'` is rejected (`a` is named both in `having` and in the select list), while `select 1 from t having 1 = 0` is legal and returns no rows. The group exists even over an empty table, so `select 1 from t having 1 = 1` returns one row whatever `t` holds. Quereus does not adopt SQLite's permissiveness of answering the rejected form with an arbitrary row's value; see [§3.3](#33-group-by-clause).

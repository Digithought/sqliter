...
- The restriction covers **every** clause evaluated after grouping — the select list, `having`, the `order by`, window specifications, and the `limit`/`offset` expressions — and reaches inside a correlated subquery written in any of them: `select a, (select count(*) from t2 where t2.b = t.b) from t group by a` names the ungrouped `t.b` and is rejected, while the subquery's *own* columns and references correlated to an *enclosing* query are unaffected. All of these raise `Column '<name>' must appear in the GROUP BY clause or be used in an aggregate function`; `having`'s own bare references keep their dedicated message (see [§3.4](#34-having-clause)), but only when the select list is itself clean — the select list is checked first, so a query whose select list *and* `having` both name an ungrouped column reports the select-list one.
...
- A query with aggregates **or a `having`**, and **no** `group by`, has one implicit group, and the same restriction applies to it with the empty set of grouping keys. The line it draws is **column-free expression versus column reference**, not aggregate versus non-aggregate — the implicit group's row carries the aggregate results and no base-table column, so:
  - An item that references **no column of the query's input** is legal beside an aggregate, because there is nothing uncovered in it. `select 'total' as label, count(*) from t`, `select 1 + 1, count(*) from t`, `select abs(-1), count(*) from t`, `select case when count(*) > 1 then 'many' else 'few' end, count(*) from t` and `select (select count(*) from t), count(*) from t` all run. This is standard SQL and matches SQLite and PostgreSQL.
  - A **column reference** is rejected, wherever it sits — bare (`select a, count(*) from t`), qualified (`t.a`), buried in an otherwise column-free expression (`'p' || a`, `length(a)`), or expanded from a star (`select *, count(*) from t`). Quereus does not adopt SQLite's permissive "bare columns" rule, which answers such a query with an arbitrary row of the group.
  - `select count(*) from t limit b` and `select count(*) as c from t having (select max(u.a) from u where u.b = t.b) = 'x'` are rejected for the same reason, and under the same message, which names the `group by` clause even though the query has none. See [§3.5](#35-order-by-clause) for which `order by` shapes sit above the aggregation and which sort the input rows below it.
...
**A `having` makes the query an aggregate query on its own.** A `having` with neither aggregates nor a `group by` is not a second `where`: as in SQLite and PostgreSQL, the query has one implicit group over all input rows and `having` filters that single group, so it returns at most one row. Everything above the aggregation is then subject to the restriction above with an empty set of grouping keys — the implicit group carries no base-table columns at all, so `select a from t having a = 'x'` and `select * from t having 1 = 1` are both rejected, while `select 1 from t having 1 = 0` is legal and returns no rows. The group exists even over an empty table, and a `where` that removes every row does not remove it either — `where` runs *below* the aggregation — so `select 1 from t where 1 = 0 having 1 = 1` still returns one row. Quereus does not adopt SQLite's permissiveness of answering the rejected form with an arbitrary row's value; see [§3.3](#33-group-by-clause).

**Such a query's `order by` may name aggregates.** `select 1 from t having 1 = 1 order by count(*)` is legal: the `having` alone makes this an aggregate query, so an aggregate is as valid a sort key here as it is for `select count(*) from t order by max(a)`. The aggregate is computed with the group and stripped from the output. (A sort key that names a base-table column instead takes the documented pre-aggregation route — see [§3.5](#35-order-by-clause).)
...
- Aggregate functions are permitted when the query is itself an aggregate query — it names an aggregate in `select` or `having`, has a `group by`, or has a `having` at all (a `having` makes one implicit group on its own, so `select 1 from t having 1 = 1 order by count(*)` is legal; see [§3.4](#34-having-clause)). An aggregate written **only** in `order by` does not itself make the query an aggregate query: `select 'k' from t order by count(*)` is rejected with `Aggregate function count not allowed in this context`, as it is in SQLite
...
- In that same no-`group by` shape, an `order by` whose terms name *no* select-list alias and *no* aggregate sorts the **input** rows before aggregation instead. That is a Quereus extension: it gives an order-sensitive aggregate a deterministic input order, so `select group_concat(b) as gc from t order by a` concatenates in `a` order. An unqualified name that IS a select-list alias outranks a same-named source column (matching SQLite, and the grouped and non-aggregate paths), which turns the extension off — `select group_concat(b) as a from t order by a desc` sorts the one output row rather than the input. Qualify the name (`order by t.a`) to keep the input sort
- The choice is made once for the **whole `order by` list**, not per term. If *any* term names a select-list alias or an aggregate, the entire list moves above the aggregation, and then *every* term must be legal up there — an aggregate, a select-list alias, or an ordinal. A bare source column among them is rejected with the same `must appear in the GROUP BY clause or be used in an aggregate function` error, because the one aggregated row does not carry it:

  ```sql
  select count(*) as c from t order by id;        -- legal: no alias, no aggregate → input sort
  select count(*) as c from t order by c, id;     -- error: `c` moves the list up, `id` is not carried
  select count(*) as c from t order by id, c;     -- error: same list, same reason — order is irrelevant
  ```
...
Both are full expressions, and both are evaluated **after** grouping. In an aggregate query they are therefore subject to the post-grouping restriction of [§3.3](#33-group-by-clause) — with or without a `group by` — and may name only grouping keys and aggregates: `select count(*) from t limit b` is rejected, `select count(*) from t limit (select count(*) from u)` is not. A correlated reference to an *enclosing* query is unaffected.

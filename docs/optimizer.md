...
- **Nothing may be registered behind it**: a rule in a later-ordered pass could rewrite a node whose estimate was re-derived here, re-opening the hole. `test/optimizer/rule-manifest.spec.ts` asserts statically that no `RULE_MANIFEST` entry targets a pass ordered after this one — today only `Pass 4: Validation` is, and it carries no rules.

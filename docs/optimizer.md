...
- **one origin relation** — `context.stats.selectivity(table, conjunct)`, exactly as for a single-table filter.
...
**Statistics gate (multi-relation path only).** `context.stats.selectivity` always returns a number for a stats-less table, because `CatalogStatsProvider` falls through to `NaiveStatsProvider`. Stamping that would replace 0.5 with 0.1 on essentially every filter-over-join, including in the many tests that never run `ANALYZE`, churning plan shapes with no information behind the change. So the multi-relation path counts a conjunct as known only when the backing statistics really exist: a single-relation conjunct needs `TableSchema.statistics.columnStats` for every column it references, and a cross-relation conjunct needs `statistics` on both tables. The single-table path is deliberately *not* gated — it keeps its existing behaviour of stamping a naive number when that is all there is.

...
Identity resolution is only as complete as `collectColumnOrigins`. **A filter over a `with` clause currently estimates nothing**: `CTEReferenceNode` mints fresh attribute ids for every column it republishes, so no attribute under a CTE reference is attributed and even a plain base column reads as having no statistics. Tracked in the fix ticket `bug-cte-reference-loses-column-origin-attribution`; see the "Known gap" section of `planner/util/column-origins.ts` for why a positional remap alone is not the fix.

...
The same isolation applies to the **common-table-expression namespace**, not only the schema search path: a stored body binds only the `with` clause it declares itself, plus schema objects on its home path — a caller's `with` clause is invisible to it, so `with t as (…) select * from v` cannot substitute the caller's `t` for the base table `v` reads. Both the schema-path and the CTE exception above are the same one: a subquery inside a body-derived expression the write-through lowering copies into the base statement still resolves against the caller. Ephemeral write targets are excluded: a CTE-name DML target (`with c as (…) update c set …`) or an inline FROM-subquery target is part of the *caller's own statement*, so it keeps the caller's CTEs along with the caller's schema path.

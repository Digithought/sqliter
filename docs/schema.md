...
**The key's `ON CONFLICT` action is emitted with the key.** A `primary key (...) on conflict replace` — declared table-level or on the column — rides whichever clause carries the key: the inline column clause for a single-column key, the table-level clause otherwise. The action is a property of the key, not of the spelling, so a table-level declaration legitimately re-parses onto the column (`ColumnSchema.defaultConflict`) and the inline branch reads that as its fallback; both emit the same text, which is what makes the round-trip a fixed point. `ABORT` is the parser's default for an absent clause and is never emitted. Without this the action was dropped on every round-trip and a `replace` key silently reverted to `abort` after a store reopen. A **non-key** column's own `on conflict` (e.g. `x integer not null on conflict rollback`) is still dropped — see the `NOTE:` on `formatColumnDef`.

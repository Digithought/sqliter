...
A **synthesized all-columns key** (a table with no declared PRIMARY KEY) emits **no** `PRIMARY KEY` clause, inline or table-level. Naming it would make a re-parse treat it as an *explicitly-declared* PK, forcing its columns NOT NULL and silently dropping a nullable declaration on a persistence round-trip; omitting it lets the re-parse re-synthesize the same key while preserving each column's declared nullability. This is the one exception to the no-`db` form's "state everything explicitly" stance, and it exists **only** because a declared `PRIMARY KEY` promotes nullability (see **Primary-key nullability** above). Once that promotion is removed the two spellings are the same key, and this omission should go with it — every key would then emit its clause and the two forms would produce identical DDL. `packages/quereus/test/table-ddl-round-trip.spec.ts` is the round-trip harness that pins the current behaviour and that the change flips.
...
Two gaps remain. A **non-key** column's own `on conflict` (e.g. `x integer not null on conflict rollback`) is still dropped — see the `NOTE:` on `formatColumnDef`. And an **all-columns key** emits no clause at all (previous paragraph), so it has nowhere to put its action either: `create table t (a integer primary key on conflict replace)` and `create table t (a integer not null on conflict replace, b text, primary key (a, b))` both lose it. That loss is *stable* — identical on the second emission — so the fixed-point property does not expose it; the round-trip harness pins it explicitly via `expectedConflictAfterRoundTrip`, and retiring the omission clears it.

Aside from the synthesized-key omission, emission is a `parse → generate → parse` fixed point: the second emission of a re-parsed schema is byte-identical to the first. `packages/quereus/test/table-ddl-round-trip.spec.ts` pins that across the primary-key shapes.

...
This document is the **hub**: storage layout, reactive hooks, query planning, schema discovery, transactions, statistics, and collation support. The subsystems large enough to read on their own live in the topic documents below.
...
| Document | Covers |
| --- | --- |
| [Catalog Persistence and Key Collation](store-catalog-persistence.md) | The bundled `CREATE TABLE` + index DDL catalog entry, when it is written, the two-phase `RENAME TABLE` protocol, and how a key column's collation reaches its physical key bytes. |
...
accomplish: the named stores read back empty when re-opened under the same names — to the
last key, however many passes an internal chunk size implies — deleting a store that was
never created is a no-op rather than an error, and every other store — sibling tables, index
stores the caller did not name, and the reserved `__stats__` / `__catalog__` stores included
— is untouched. Releasing
...
How a table's definition reaches the catalog store — the bundled `CREATE TABLE` + `CREATE
INDEX` (+ `alter index … set tags`) entry keyed `{schema}.{table}`, when that entry is
written, the two-phase `RENAME TABLE` protocol and the residues it accepts, and how a key
column's collation reaches the physical key bytes (including OBJECT-class encoding and
index-derived UNIQUE enforcement) — lives in
[store-catalog-persistence.md](store-catalog-persistence.md). Views and materialized views
are engine-level objects that never reach a module hook; they persist under reserved-prefix
keys through a schema-change subscription, described in
...
- `ALTER TABLE ... ALTER COLUMN <non-pk-member> SET DATA TYPE`, for any move between two *different* logical types — the physical representation need not change (every row's value is re-parsed and re-stored in the new type's canonical form). Retyping a **primary-key member** to a different logical type is rejected with `CONSTRAINT` before anything is scanned or written: a retype moves the key's *type* and comparator, not only its bytes. (The bytes alone would now follow — the store applies the value rewrite before the re-key, so `rekeyRows` reads the rewritten row — but nothing exercises that path.) The engine refuses this for every backend already (see [SQL DDL § ALTER TABLE](sql-alter.md#27-alter-table-statement)); the store repeats the check for direct module calls, as the memory backend does
- `ALTER TABLE ... ALTER COLUMN ... SET NOT NULL`, when existing rows hold NULL and a literal `DEFAULT` backfills them. On a **primary-key member** (nullable only when the table's identity is its full column set) the backfill moves the key values, so it is also a re-key: the value rewrite runs first, then `rekeyRows` recomputes every data key from the rewritten row and the secondary indexes are rebuilt once — the backfilled row is findable by its new key, and an `INSERT` of the same values collides with it

Validation that can reject the statement runs *before* the commit wherever possible, reading the buffered-plus-committed view so it sees this transaction's own rows: the NULL probe of `SET NOT NULL`, the convertibility probe of `SET DATA TYPE`, the `NOT NULL`-without-`DEFAULT` probe of `ADD COLUMN`, the non-PK UNIQUE re-validation — run for `SET COLLATE` and, over the *converted* values, for the two value-rewriting arms (`SET DATA TYPE`, `SET NOT NULL` backfill), whose rewrite can collapse two distinct values onto one — and both primary-key probes of *every* PK re-key, `SET COLLATE` on a PK member, the `SET NOT NULL` backfill of a PK member (both probes judge the *converted* rows) and `ALTER PRIMARY KEY` alike (`StoreTable.validateRekeyedPrimaryKey`: `CONSTRAINT` over the transaction's effective rows, `BUSY` over the committed rows a rollback must restore). All of these leave the transaction intact when they reject. Validation that cannot be separated from the rewrite runs after the commit, so its error arrives with the enclosing transaction already committed. Storage is still left untouched in that case; only the transaction is gone. One check remains in this category: the `NOT NULL` check on a value produced by an `ADD COLUMN` backfill expression (it runs per row, as the rows are rewritten). `rekeyRows`' own duplicate-key pass, for every re-keying statement, is now only a backstop behind the pre-commit probes above, not a rejection path a caller can reach.
...
instance. See
[Committed-Snapshot Reads](module-committed-reads.md)
...
The substituted count is floored at 1. That floor used to be load-bearing: `rows: 0` on a fully-claimed plan was the access-plan protocol's *"this predicate is unsatisfiable"*, and `rule-select-access-path` replaced the whole table access with a static empty relation on it — deleting the read of a table that is empty when the plan is built but written by the same statement. The fold now reads the explicit `provablyEmpty` proof, which this module never sets, so the floor is purely a cost-model guard: a 0-row estimate prices below every rival plan and distorts join ordering, cache admission and sort costing above the access. Consequence, unchanged: a genuinely empty store table is costed as though it held one row, not zero.
...
- NOTE: the two largest `StoreTable` layers have both passed the ~1,000-line seam
  (`store-table-scan.ts` 1,401, `store-table-base.ts` 1,120 — `wc -l`, 2026-08-11).
  Splitting them: backlog `debt-oversized-source-files`. Scan-layer seams — the multi-seek
  group (`decodeMultiSeekTuples` / `orderTupleValues` / `scanMultiSeek` /
  `scanMultiSeekPrimary`) and the row-resolution group (`produceIndexEntries` /
  `resolveIndexEntries` / `resolveRowBatch`); the base's is the statistics block. Until it
  lands, put new scan-side logic in a collaborator (`pk-key-resolution.ts`,
  `key-builder.ts`, `json-key.ts`) rather than growing these two.

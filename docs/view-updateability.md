...
Three related defects at the same symptom but different code sites remain open: a body lineage sub-select that qualifies its correlation with the base table's *name* (`fix/bug-view-write-lineage-subquery-base-table-qualifier`); the shadow-analysis pass that sizes up a fragment's `from` sources against one fixed schema (`fix/bug-view-write-subquery-shadow-analysis-wrong-schema`); and a set-operation body's non-leading legs, whose branch **body** (not its fragments) loses the declared `with schema` path because the parser attaches that clause to the leading leg only (`fix/bug-setop-right-leg-write-drops-declared-schema-path`).

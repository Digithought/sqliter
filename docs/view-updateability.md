...
Two related defects at the same symptom but different code sites remain open: a body lineage sub-select that qualifies its correlation with the base table's *name* (`fix/bug-view-write-lineage-subquery-base-table-qualifier`), and a body sub-select that reads the body's own `with` clause (`fix/bug-view-write-body-cte-not-carried-into-lowering` — the marker clears the caller CTE namespace, so this now always errors rather than possibly binding a caller table of that name).

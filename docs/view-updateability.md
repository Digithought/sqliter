...
Clearing the caller's CTE namespace is only half of it: the body's **own** `with` clause has to take its place, or a copied fragment that reads a body-local block has nothing to bind to. So the same stamp carries that clause on `AST.SelectStmt.storedBodyCTEs`, and `buildSelectStmt` builds those definitions on the home context (`buildStoredBodyCTEs`, `planner/building/select-context.ts`) as the fragment's parent CTE namespace — the fragment's own `with` clause still merges on top and shadows a body-local name. Without the carry a write through `create view v as with c as (…) select … from a where id in (select id from c)` either failed with `Table 'c' not found` or, when a *real* table `c` existed, silently bound that table instead and wrote zero rows while the read of the same view returned them.

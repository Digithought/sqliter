...
the leaves' costs; see `debt-node-self-cost-reads-the-blank-estimate` in `tickets/backlog/`.
...
#### When it becomes a fan-out (batched seeks)

One seek per outer row is the right shape on an in-process table. On storage reached over a network it is one round trip per outer row, so each index-nested-loop orientation whose seek **provably** returns at most one row per outer row is additionally offered as a **one-branch `FanOutLookupJoinNode` in `batched` outer mode** — the same rebuilt seek, driven by the fan-out's cross-row pipelined runtime (`runFanOutLookupJoinBatched`, see [Fan-out batched outer](#fan-out-batched-outer)) so many seeks are in flight at once and the module's round trip is amortized across them. The output is indistinguishable from the one-at-a-time plan: the driver re-orders completed rows back into outer order before emitting.

- **Decided inside the comparison, as a priced candidate** — not by a later rewrite rule. A post-pass would only ever see index-nested-loop joins that already won at one-seek-at-a-time prices, and under latency those mostly lose to hash join. Batching changes the price, so the price has to be in the comparison. With `expectedLatencyMs = 25`, 20,000 outer rows, one row per seek and the shipped constants, the inner-table size at which the seek plan beats hash join moves from about 1.3 million rows to about 128,000 (evaluated by hand from the formulas below, not a benchmark).
- **The at-most-one proof** (`provesAtMostOne` in `index-nested-loop.ts`) is a proof, not the module's row estimate: an `atMostOne-*` branch that yields a second row throws `CONSTRAINT` at runtime, so a module that estimates 1 and returns 2 would turn a working query into an error. It reads the full constraint list offered to the module (`offered.combined` — the synthesized join-key equalities plus the displaced leaf's pushed predicates), every one of which the candidate's own correctness argument guarantees is enforced somewhere in the rebuilt inner (by the seek, by a reattached or re-applied Filter, or by the ON condition kept above). A Filter only removes rows, so a constraint enforced by a filter counts the same as one enforced by the index. The claim holds when some key the table enforces uniqueness on — the primary key, a non-partial UNIQUE constraint, or a non-partial UNIQUE index — has **every** column pinned by a deterministic equality whose comparison collation is at least as fine as the key's enforcement collation: the predicate compares BINARY (the finest relation there is), or the two normalize to the same name. A coarser predicate (`NOCASE` join key over a `BINARY` unique column) admits `'a'` and `'A'`, two distinct stored rows that both match, and declines; ranges, `IN` and `OR_RANGE` never contribute; a partial key only constrains the rows its WHERE admits. The refinement test is the shared `collationRefines` in `util/comparison.ts`, and the proof reads an equality's collation only from a real comparison node: the resolver answers BINARY for a shape it does not recognize, which is a safe decline for the access-path rule but the strongest possible claim here, so an unrecognized `=` producer declines instead. Over-fetch is harmless — a `COARSER_SAFE` seek's residual Filter re-applies the exact comparison — and the finer-index case never reaches the proof (it degrades to a scan and the candidate is already declined). A user predicate that was **not** pushed (a Filter above a walk leaf) is not in the offered list and is not consulted; `feat-index-nested-loop-offer-filter-predicates` (backlog) would fold those in.
- **Construction**, for outer `O`, rebuilt inner `I` and ON condition `C`: one branch `{ child: Filter(I, C), mode: 'atMostOne-left' | 'atMostOne-inner', outputAttrs: I.getAttributes(), concurrencySafe: I.physical.concurrencySafe !== false }` under `FanOutLookupJoinNode(O, [branch], concurrencyCap = 1)`. The ON condition becomes a Filter inside the branch (a branch has no residual slot — the same construction `rule-fanout-lookup-join` uses for its spine branches); it cannot add rows, so it cannot break the at-most-one claim, and for a LEFT join filtering the single row away yields an empty branch, which `atMostOne-left` NULL-pads. The un-mirrored orientation pins the logical join's attributes (`[...left, ...right]`, right nullable-widened for LEFT — exactly the fan-out's `[...outer, ...branch]` layout), the mirrored one lets the node derive `[...right, ...leftRebuilt]`. `concurrencyCap` is 1 because there is one branch; the cross-row budget is the batched driver's `outerBatchConcurrency`.
- **One function owns the flip.** The serial node is handed to `toBatchedOuter` (exported by `rule-fanout-batched-outer`), which applies every batched gate — latency threshold, outer cardinality, outer `concurrencySafe`, no side effects — and wraps the outer in the load-bearing `EagerPrefetchNode`. If it declines there is **no** batched candidate for that orientation; a serial one-branch fan-out is never entered into the comparison (strictly worse than the join it replaces: per-row forking, no overlap). Building the node and asking the gates in one step means the price and the plan can never disagree; at zero inner latency the rule does not even build the node, since the batched price equals the serial one and a strict `<` could never pick it.
- **Price**: `batchedIndexNestedLoopJoinCost` below, with the in-flight count `min(outerBatchConcurrency, maxOuterReadAhead)` — `deriveReadAhead` at one branch, stated at both ends — plus one open of the outer side, the same charge the serial orientation pays.
- **Tie order**: each batched candidate sits directly after its serial counterpart, un-mirrored pair before mirrored pair — `nested-loop, hash, merge, index-nl, index-nl-batched, index-nl-mirrored, index-nl-mirrored-batched`, all strict `<`. A batched candidate is never dearer than its serial one (only the latency term is divided), so it wins its orientation whenever it exists and more than one seek is in flight, and a tie between the un-mirrored batched and the mirrored serial keeps the spelled drive side.
- **Not offered** for `semi` / `anti` (the fan-out has no semi/anti mode), for `exists … as` flag joins (no existence-flag support), or when the proof fails — those keep the plain index-nested-loop join. A `USING` join that takes this path loses its `usingColumns` EXPLAIN label (the fan-out has no equivalent; attribute layout is unaffected). The row estimate becomes the outer's cardinality (`FanOutLookupJoinNode` reports the outer for an at-most-one branch) where `JoinNode` used `estimateJoinRows`.
- **Later rules**: `rule-fanout-batched-outer` re-visits the node and declines on its idempotence gate (`outerMode === 'batched'`); `materialization-advisory` sees the `EagerPrefetchNode` already in place.
...
- Index-nested-loop, batched: `outerRows × (1.0 + 0.5 + rowsPerSeek × 0.3 + perSeekLatencyMs / max(1, inFlight))` — the engine-side per-row work is unchanged (single-threaded); only the storage latency is amortized across the seeks in flight
...
| --- | --- | --- | --- |
| plain nested loop | left | one per outer row, unless the inner is (or is about to be) cached | `leftLatency + (opensOnce ? rightLatency : leftRows × rightLatency)` |
...
| index-nested-loop, batched (either orientation) | as its serial counterpart | `inFlight` seeks overlapped | same outer open as its serial counterpart — the per-seek term, divided by the in-flight count, is inside `batchedIndexNestedLoopJoinCost` |

"opensOnce" is `nestedLoopRightOpensOnce`, exported by `rule-nested-loop-right-cache`: the right side is already materialized, or that rule (later in the same pass) is about to wrap a pure, uncorrelated, small-enough inner side in a `CacheNode` and turn N re-opens into one open plus N buffer replays. The selection rule calls that predicate rather than restating its gates, and skips the call entirely when the right's latency is 0 — the predicate's size gate walks the whole right subtree, so keeping it off the zero-latency path matters.

Note it is deliberately the **open-count** question and not plain cacheability (`canCacheNestedLoopRight`, the same file's other export). The two differ exactly on an already-cached right side, which is not cacheable *again* but does open once; selection re-visits a join after the cache rule has rewritten it, so asking the cacheability question there re-priced the loop as if the cache were absent and converted it to a hash join, discarding the cache.

Two cases the predicate still answers coarsely, both inert until a module declares latency. An impure right side that `rule-mutating-subquery-cache` will wrap reads as "opens per row" through the purity gate, so the plain nested loop is over-charged — unreachable in practice, since index-nested-loop declines an impure inner outright and the hash build/probe swap refuses too, so the over-charge has no cheaper rival to hand the win to. And a semi/anti join breaks out of the inner scan on the first match, so its cache buffer only lands after the first *unmatched* outer row: a match-heavy semi join can re-open the inner per outer row while this charges it once, optimistic by at most `(outerRows - 1) × latency` — in the same direction the work term is already pessimistic, since that charges a full inner scan per outer row for a loop that breaks early.
...
- **`outer.estimatedRows >= tuning.parallel.batchedOuterMinRows`** (default 256, ≈ 4× `maxOuterReadAhead`) — large outer cardinality so cross-row overlap dominates the reorder-buffer + per-row-fork overhead. An **unknown estimate fails the gate** (never flip on a missing statistic). Single-source relays carry the leaf's `physical.estimatedRows` upward (`physicalSourceRows`), and the set operations, gathers, CTE nodes and write pipeline now relay too, but not every wrapper does — so the rule reads the node's own estimate then descends single-relation pass-throughs to recover the leaf's `physical.estimatedRows`. A never-`ANALYZE`d leaf reports `undefined` and fails the gate outright, so the rule stays inert on unmeasured tables independent of the latency gate.
...
**Two callers, one flip.** The gates, the prefetch wrap and the node rebuild live in the exported `toBatchedOuter(node, tuning.parallel)`; the rule is a thin wrapper over it. `rule-join-physical-selection` calls the same function directly to turn a proved-at-most-one index-nested-loop candidate into a priced one-branch batched fan-out (see [Index-Nested-Loop Join → When it becomes a fan-out](#when-it-becomes-a-fan-out-batched-seeks)). One owner means the two callers cannot drift, and no caller can obtain batched mode without the isolation wrap. The idempotence gate is what lets this rule re-visit a node the selection rule already flipped.

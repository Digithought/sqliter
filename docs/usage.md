...
**How stale is "slightly stale"?** The state a committed read serves is pinned when that read *starts*, and held for the whole of it — so a write that lands anywhere (this process or, for a networked module, another machine) while the read is running is not in its rows. It is the *next* committed read that sees that write: a module is only allowed to declare itself for this path if each new read re-pins to whatever is committed at the moment it begins, which is what makes a repeated read converge. A module that pinned once and never re-pinned would go arbitrarily stale instead, and a poll against it would never converge — that is non-conformance, and [the conformance harness](module-committed-reads.md#proving-it-the-conformance-harness) is how you check a module before trusting it here.

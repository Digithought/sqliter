...
- **`providesOrdering` also claims NULL placement, and the engine's placement is absolute.** `ORDER BY` puts NULLs FIRST for *both* directions — `compareSqlValues` ranks NULL below every other storage class and the direction flip is applied after (`orderByNullResult`, `util/comparison.ts`). Most key encodings agree on an ascending column and *disagree* on a descending one, because inverting the bytes sends the NULL tag to the end of the walk. So a DESC key column may only be claimed when no NULL can reach it: the column is declared `NOT NULL`, the plan pins it to a single value with an equality (which never matches NULL), or a pushed filter on it excludes NULLs (`=`, `IN`, any range, `IS NOT NULL`) and is enforced by the seek window, the module's own residual, or the `Filter` the engine keeps for an unhandled filter. Truncate the claim at the first descending column that fails all three — the rows are otherwise returned at the wrong end with the Sort already deleted. Do not re-derive it: `nullSafeOrderingPrefixLength(tableInfo, request, keyColumns, orderPreservingPrefix, pinnedCols)` is exported from `@quereus/quereus` and is what both shipped backends use. It takes the key columns as an `IndexColumnSchema[]`, so an index's `columns` and a table's `primaryKeyDefinition` both go in unchanged — **a nullable descending primary-key member is exposed by exactly the same reasoning, with no `CREATE INDEX` involved.** A caller that claims a required ordering verbatim (all or nothing) wants the boolean `nullSafeOrderingPrefixLength(…) === matchedLength`; truncation only matters where an index's own ordering is advertised with no request to match. The same gate applies to `monotonicOn`: a `direction: 'desc'` claim asserts the same physical order, so void it (and `supportsAsofRight`, which implies it) whenever the leading key column is the one that fails.

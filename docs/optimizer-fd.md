...
The fourth, **CHECK / assertion extraction** (`planner/analysis/check-extraction.ts` — `handleEquality`'s col=col arm and `recognizeGuardedBody`'s `valueEquality` mirror pair), is **not** gated yet and is unsound today: `create table ck (d timespan, s text, check (d = s))` lifts `equivClasses: [[d, s]]` onto the TableReferenceNode, `rule-predicate-inference-equivalence` reads it straight off the source, and `where d = 'PT1H'` gains an inferred `s = 'PT1H'` that drops a row storing `'PT60M'`. Unlike the filter-side EC (which merges into the same node as the pin), nothing makes this one dormant. Tracked as `tickets/fix/check-derived-equivalence-ignores-semantic-ordering`.
...
The gate on `extractEqualityFds` also closes the guard-discharge route through equivalence classes: `clauseEntailed` discharges an `eq-literal` guard from any EC peer pinned to that literal, so a false `{d, s}` class would have discharged an `s = 'PT1H'` guard from `d = 'PT1H'` and activated a partial-UNIQUE `kind: 'unique'` FD that does not hold. `clauseEntailed` reads whatever EC list reaches the Filter, so the ungated CHECK site above can still feed it one — a second arm of the same tracked bug.

...
`semanticOrderingsAgree` compares `LogicalType` by object identity, so two distinct instances of a same-named type over-decline. That is the safe direction and is shared by all three sites.

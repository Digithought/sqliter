...
- **Join equi-pairs**: `extractEquiPairsFromCondition` (logical facts: key coverage, FD/EC propagation, FK alignment, join elimination, the coverage prover) requires value-discriminating equality; `rules/join/equi-pair-extractor.ts` (physical hash/merge/bloom selection) requires *matched* operand collations — a matched-collation pair is immune to the emitters' resolution-order differences and its coverage claims compare under the covered key's own enforcement collation. Mismatched conjuncts demote to the residual, where the canonical scalar comparison evaluates them. `COLLATE`-wrapped operands never form pairs in either extractor (operands must be bare column references) — a deliberate, pinned exclusion.
  - The two extractors also **differ on semantic ordering**, and only the physical one is gated today. `equi-pair-extractor.ts` additionally requires the pair's two sides to agree on semantic ordering (`semanticOrderingsAgree` — see `docs/optimizer-joins.md` § Physical Join Algorithm Selection); `extractEquiPairsFromCondition` does not, so a `timespan_col = text_col` conjunct still mints a *value*-equality fact even though the rows it matches (`'PT1H'` against `'PT60M'`) are not value-equal. That over-claim is observable through `rule-predicate-inference-equivalence`, which substitutes a constant across the resulting equivalence class and drops rows. Tracked as `tickets/fix/equality-fact-extraction-ignores-semantic-ordering`.

...
| `SeqScanNode` / `IndexScanNode`            | Pass child FDs/ECs through unchanged.                                                                                                |
| `IndexSeekNode`                           | Pass child FDs/ECs through unchanged, plus **one** claim of its own: a non-range seek on the primary key whose seek keys pin every PK column **exactly once** returns at most one row, folded in via `addSingletonFd` (`∅ → all_cols`). "Exactly once" excludes a multi-seek (`where id in (1, 2, 3)` supplies one key per IN member), which is detected via `accessPathPlan(filterInfo.accessPath) === 'multiSeek'` — a composite-key multi-seek that happens to reduce to a single tuple is declined too, conservatively. See [Optimizer Costing § What a physical seek node reports](optimizer-costing.md). |
...
**Capability gate on FK trust ([OPT-059](invariants.md#opt-059--fk-derived-existence-is-capability-gated)).** A declared foreign key seeds an IND only when **neither side's** owning vtab module declares the `permitsOrphanedForeignKeyRows` capability (`vtab/capabilities.ts`). An orphan-permitting backend can hold a child row whose FK value matches no parent — via a grandfathering `ALTER TABLE … ADD CONSTRAINT`, or a parent deleted through a path the engine never sees — so its declared FKs are declarations of intent, not inclusion facts. Both producers gate: `seedTableForeignKeyInds` seeds nothing for a gated child (and skips per-FK when the resolved parent's module is gated), and `lookupCoveringFK` returns no match, so every structural consumer (join elimination's INNER arm, the semi/anti-join folds, `atMostOne-inner` fan-out, the coverage prover's structural fallback) declines by the same mechanism. Only the *existence* (≥1 match) conclusion is gated — the at-most-one claim (`checkFkPkAlignment`, `util/key-utils.ts`) follows from the parent key's uniqueness, not inclusion, and stays available under the cap.

**Soundness boundary (load-bearing).** A false IND (**over-claim**) is unsound — it asserts a row exists that does not, which would silently mis-prove coverage downstream. A missing IND (**under-claim**) only forgoes an optimization (the fallback path runs — safe). Therefore **every propagation rule is conservative: drop when unsure.** This matches the coverage prover's "a false `Covers` is unsound ⇒ be conservative" bar and the RI-trust assumption `ind-utils.ts` already makes (declared FKs treated as hard inclusion dependencies when neither side's module permits orphans — see *Capability gate on FK trust* above; `pragma foreign_keys` defaults on). The `test/optimizer/inclusion-dependencies.spec.ts` property/law harness is the empirical backstop: for representative optimized plans it materializes each relational node and asserts every propagated IND actually holds (each row's `cols` projection, excluding NULL-rejected rows, is present in the target's `targetCols` projection) — the IND analogue of the Key Soundness harness.

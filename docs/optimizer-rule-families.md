...
- **Value expressions are values, not comparisons** (ticket `or-collapsed-in-value-exprs`): a constraint's `valueExpr` — scalar, or one array element per `IN` member — always evaluates to the constrained *value*. A consumer materializes these as index seek keys, which are evaluated before any row of the sought table is read, so a key may reference other relations (correlated pushdown) but never a column of the table being sought. `collapseBranchesToIn` used to fill a literal OR branch's slot with the branch's whole comparison (`col = 10` instead of `10`), which any module claiming the collapsed `IN` as a multi-seek then tried to evaluate row-free — surfacing as "No row context found for column …" from deep inside expression evaluation. `IndexSeekNode`'s constructor now checks every seek-key array against the sought table's own attribute ids (`assertSeekKeysRowIndependent`, in `planner/nodes/table-access-nodes.ts`) and raises a named internal planner error, so the class fails at plan time instead — at the node rather than at any one producer, which also covers clone and `withChildren` re-key paths. See [Invariants § OPT-061](invariants.md#opt-061--a-seek-key-never-references-a-column-of-the-table-being-sought).
...
Foreign keys are inclusion dependencies — `child.fk ⊆ parent.pk` — and three optimizer rules exploit them to remove parent-side access entirely. All three run in the Structural pass, after `rule-subquery-decorrelation` has materialized `EXISTS / NOT EXISTS / IN` as semi/anti joins — and after `semijoin-existence-recovery` has recovered a semi/anti join from a probe-only outer-join `exists … as` flag (`where flag` / `where not flag`), which the folders treat identically (a covering-FK semi recovered from `where flag` folds to L just as a decorrelated `EXISTS` semi does). The shared util `lookupCoveringFK` walks `TableSchema.foreignKeys`, matches the equi-pairs against the FK's declared *positional* pairing (`fk.columns[i] → resolveReferencedColumns(fk, parent)[i]`, resolved from the FK's parent column names against the parent's current schema — a permuted equi-pair set on a composite FK such as `(fa = b AND fb = a)` against `FOREIGN KEY (fa, fb) REFERENCES p(a, b)` is **not** covered and the rule abstains), and reports the matched FK plus whether any child column is nullable; `isRowPreservingPathToTable` guards against parent-side filters/limits/distincts that would invalidate the IND under filtering. `lookupCoveringFK` is also where the `permitsOrphanedForeignKeyRows` capability gate lives ([OPT-059](invariants.md#opt-059--fk-derived-existence-is-capability-gated)): it reports no match when either side's owning vtab module declares the capability, so every existence consumer in this family declines together. The *at-most-one* helper `checkFkPkAlignment` is deliberately ungated — ≤1 match follows from the parent key's uniqueness, not from inclusion.

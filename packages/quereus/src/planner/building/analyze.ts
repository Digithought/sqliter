...
import { isCommittedSchemaRef, resolveTableSchema } from './schema-resolution.js';
...
export function buildAnalyzeStmt(ctx: PlanningContext, stmt: AST.AnalyzeStmt): PlanNode {
	if (stmt.tableName) {
		...
		// The `committed` pseudo-schema is dropped here for the same reason
		// resolveTableSchema drops it: it is not a real schema, so the name behind
		// it resolves through the search path.
		const lookupSchema = isCommittedSchemaRef(stmt.schemaName) ? undefined : stmt.schemaName;
		const item = ctx.schemaManager.findSchemaItem(stmt.tableName, lookupSchema, ctx.schemaPath);

...
export function emitCreateAssertion(plan: CreateAssertionNode, _ctx: EmissionContext): Instruction {
	...
	async function run(rctx: RuntimeContext): Promise<SqlValue> {
		...
		// Planned the same way the commit-time evaluator plans it — same home-schema
		// path, same analysis stage, same hoist suppression (see
		// `planAssertionBodyForAnalysis`) — and walked with the same collector the
		// evaluator's `extractBindings` walks with, so the informational list and the
		// set enforcement actually derives cannot disagree about what the body reads.
		...
		try {
			const analyzed = planAssertionBodyForAnalysis(rctx.db, violationSql, plan.schemaName);
			assertionSchema.dependentTables = Array.from(

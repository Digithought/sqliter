...
const MEMORY_ONLY_FILES = new Set([
  ...
  '41.7.4-alter-column-retype-semantic-memory.sqllogic',  // Pins the memory module's same-storage-class retype contract (text ↔ timespan, text collate nocase → date). The store covers the TIMESPAN half via its own key-transform guard, but not the date/collation half, which turns on comparator identity rather than a key transform

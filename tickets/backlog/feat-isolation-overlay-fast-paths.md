...
| 3 | Existence check via point lookup | **Shipped**, and beyond the sketch. `rowExistsInUnderlying` (`flush.ts:111`) drives the underlying's primary-key index through `makePkPointLookupFilter` (`filter-info.ts:21`), and the flush additionally hoists every probe ahead of the first write of that table (`flush.ts:69-73`) because an underlying module is not obliged to serve index-driven reads over its own uncommitted writes. NOTE: "point lookup" is a *request*, not a guarantee — a module that cannot seek those columns legitimately answers with a superset, so the probe has to verify the row it gets back (in-flight: `bug-isolation-underlying-pk-lookup-trusts-unfiltered-row`), and against such a module it costs a scan per probe rather than O(log n). |

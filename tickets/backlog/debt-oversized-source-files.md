...
files:
  ...
  - packages/quereus/src/planner/building/select-aggregates.ts # 1,486 lines (`wc -l`, 2026-08-12, after the select-list group-key redirect; 1,451 on 2026-08-11, 1,400 before the ungrouped-aggregate ORDER BY alias change, 1,296 before the grouping-key-alias change) — noticed during the window/group-key-alias review; GROUP BY key indexing and redirection, the aggregate output scope, HAVING construction, and the final grouped projection are four separable concerns in one file, and the redirect half now has three prospective callers (select list, window phase, ORDER BY)

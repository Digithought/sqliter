...
files:
  ...
  - packages/quereus/src/planner/building/select-aggregates.ts # 1,451 lines (`wc -l`, 2026-08-11, after the ungrouped-aggregate ORDER BY alias change; 1,400 before it, 1,296 before the grouping-key-alias change) — noticed during the window/group-key-alias review; GROUP BY key indexing and redirection, the aggregate output scope, HAVING construction, and the final grouped projection are four separable concerns in one file

...
files:
  ...
  - packages/quereus/src/planner/analysis/constraint-extractor.ts   # 1,695 lines (`wc -l`, 2026-09-01; 1,647 on 2026-08-11) — noticed during the relation-key review; predicate normalization, covered-key derivation, `analyzeRowSpecific`, and `TableInfo` construction are four separable concerns in one file
  - packages/quereus/src/planner/building/select-aggregates.ts # 1,683 lines (`wc -l`, 2026-09-01, after the ungrouped column-free select-item review); 1,675 lines (2026-08-23, after the having-only aggregate-query review; 1,668 earlier the same day after the ungrouped-aggregate coverage-check split; 1,645 earlier the same day; 1,630 on 2026-08-12, after the post-aggregate redirect choke point + finished-plan boundary check; 1,486 earlier the same day after the select-list group-key redirect; 1,451 on 2026-08-11, 1,400 before the ungrouped-aggregate ORDER BY alias change, 1,296 before the grouping-key-alias change) — noticed during the window/group-key-alias review; GROUP BY key indexing and redirection, the aggregate output scope, HAVING construction, and the final grouped projection are four separable concerns in one file, and the redirect half now has five callers routed through one entry point (select list, window phase, ORDER BY, HAVING, pre-window sort) plus a plan-walking coverage checker. `buildAggregatePhase` alone is 192 lines (lines 27-218, `sed -n '27,218p' | wc -l`, 2026-08-23) and is the natural first cut: HAVING-aggregate collection, ORDER-BY-aggregate collection, the sort-placement decision, AggregateNode construction, and the needsFinalProjection decision are five sequential phases with no shared mutable state beyond `currentInput`
  - packages/quereus/src/planner/building/insert.ts          # 1,146 lines (`wc -l`, 2026-09-01; ~1,050 before the write-form conversion ticket) — noticed during that ticket's review, after its row-expansion function was already decomposed into stage helpers; INSERT statement building, the row-expansion chain, upsert clause planning, and constraint/FK wiring are separable concerns in one file
...
**access-path cost model** (`findBestAccessPlan`, `evaluateIndexAccess`,
`adjustPlanForOrdering`, `evaluateOrderingOnlyPlans`, `findEqualityMatches` /
`findRangeMatch` / `findOrRangeMatch`, and now `estimateEqualityRows` /
`equalityTupleSelectivity` / `columnStatsFor`). The costing half is the natural
extraction, and the store package has already done exactly that split — its equivalent
lives in its own file, `store-module-access-plan.ts`. The two halves also now duplicate
one number across packages: the memory module's `EQ_SELECTIVITY_WITHOUT_STATS` is a
hand-kept copy of the store's `ARM_SELECTIVITY.eq`, with a comment saying so. A shared
home for that constant is the obvious thing to put in the extracted file (the store
